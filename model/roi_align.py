...
def crop_and_resize(masks: torch.Tensor, boxes: torch.Tensor, mask_size: int) -> torch.Tensor:
    """
    ...
    """
    assert len(boxes) == len(masks), "{} != {}".format(len(boxes), len(self))
    device = masks.device
